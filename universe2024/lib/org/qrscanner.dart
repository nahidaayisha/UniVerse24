    ...
    controller.scannedDataStream.listen((scanData) {
      setState(()  {
        ...
        try{
          ...
          getDetailsOfScanned(context,result!.code.toString(),);
          controller.resumeCamera();
...
void getDetailsOfScanned(BuildContext context, String code) {
  ...
  db.collection('users').doc(code.toString()).snapshots().listen((DocumentSnapshot ds) {
      ...
      db.collection("REGISTRATIONS")
        ...
        if (value.docs.isNotEmpty) {
          for (var element in value.docs) {
            db.collection("REGISTRATIONS").doc(element.id).set(
                {"scanned_status": "YES"}, SetOptions(merge: true));
          }

          const snackBar = SnackBar(
              backgroundColor: Colors.red,
              duration: Duration(milliseconds: 3000),
              content: Text("Scanned Succesfully", textAlign: TextAlign.center,
                softWrap: true,
                style: TextStyle(
                    color: Colors.white
                ),
